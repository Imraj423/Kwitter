...
import handleMessageSubmit, {handleComposeMessageProfileEnter} from "./CreateMessage";
...
dispatch(unLike(likeId)).then(()=>dispatch(getMessages()))
  ...
  return (
    <>
      ...
      >
        <Input
          onChange={this.props.handleMessageSubmit}
          style={{ border: "1px gray solid", width: "358px" }}
        >
          Enter Comment...
        </Input>
        <Button onClick={this.handleComposeMessageProfileEnter}>Kweet!</Button>
      </div>
      {kweets.length > 0 &&
